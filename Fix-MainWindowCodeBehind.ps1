...
$csPath = Join-Path $RootPath "src\App\MainWindow.xaml.cs"
if (-not (Test-Path $csPath)) { throw "MainWindow.xaml.cs not found at $csPath" }

Copy-Item $csPath "$csPath.bak" -Force
Write-Host "[INFO] Backed up existing MainWindow.xaml.cs to $csPath.bak"
...
Set-Content -Path $csPath -Value $newCode -Encoding UTF8
Write-Host "[OK] MainWindow.xaml.cs replaced with clean version. Rebuild the solution."
