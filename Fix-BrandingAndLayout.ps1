param(
    [Parameter(Mandatory=$true)]
    [string]$RootPath,
    [Parameter(Mandatory=$true)]
    [string]$LogoSource # Path to your CyberArk logo file (jpg/png)
)

$ErrorActionPreference = "Stop"

# Paths
$appProj = Join-Path $RootPath "src\App\CyberArk.Migrator.App.csproj"
$xamlPath = Join-Path $RootPath "src\App\MainWindow.xaml"
$resDir   = Join-Path $RootPath "src\App\Resources"
$logoDest = Join-Path $resDir "CyberArkLogo.png"

# 1) Ensure Resources folder exists
if (-not (Test-Path $resDir)) { New-Item -ItemType Directory -Path $resDir | Out-Null }

# 2) Copy logo into Resources as PNG
Write-Host "[INFO] Copying logo into Resources..."
Copy-Item $LogoSource $logoDest -Force

# 3) Backup files
Copy-Item $appProj "$appProj.bak" -Force
Copy-Item $xamlPath "$xamlPath.bak" -Force

# 4) Update .csproj to embed the logo
$appProjContent = Get-Content $appProj -Raw
if ($appProjContent -notmatch "CyberArkLogo.png") {
    $insert = @"
  <ItemGroup>
    <Resource Include="Resources\CyberArkLogo.png" />
  </ItemGroup>
"@
    $appProjContent = $appProjContent -replace '(</Project>)', "$insert`n</Project>"
    Set-Content -Path $appProj -Value $appProjContent -Encoding UTF8
    Write-Host "[OK] Embedded CyberArkLogo.png into project file."
}
else {
    Write-Host "[INFO] Logo already referenced in project file."
}

# 5) Replace MainWindow.xaml with branded layout
$newXaml = @"
<Window x:Class="CyberArk.Migrator.App.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="CyberArk Migrator" Height="500" Width="900"
        Background="#F5F8FA">
    <Grid>
        <Grid.RowDefinitions>
            <RowDefinition Height="Auto"/>
            <RowDefinition Height="*"/>
        </Grid.RowDefinitions>

        <!-- Header -->
        <DockPanel Grid.Row="0" Background="#004B8D" Height="60">
            <Image Source="Resources/CyberArkLogo.png"
                   Height="40" Margin="10,10,20,10"/>
            <TextBlock Text="CyberArk Migrator"
                       VerticalAlignment="Center"
                       Foreground="White"
                       FontSize="22"
                       FontWeight="Bold"/>
        </DockPanel>

        <!-- Body -->
        <Grid Grid.Row="1" Margin="10">
            <Grid.ColumnDefinitions>
                <ColumnDefinition Width="280"/>
                <ColumnDefinition Width="*"/>
            </Grid.ColumnDefinitions>

            <!-- Controls Panel -->
            <StackPanel Grid.Column="0" Margin="0,0,10,0">

                <!-- Features -->
                <GroupBox Header="Features" Margin="0,0,0,10">
                    <StackPanel>
                        <CheckBox Name="chkDelinea" Content="Use Delinea"/>
                        <CheckBox Name="chkBeyondTrust" Content="Use BeyondTrust"/>
                        <CheckBox Name="chkOsirium" Content="Use Osirium"/>
                        <Separator/>
                        <CheckBox Name="chkPCloud" Content="Import to Privilege Cloud"/>
                        <CheckBox Name="chkSelfHosted" Content="Import to Self-Hosted"/>
                        <CheckBox Name="chkDryRun" Content="Dry Run (no changes)" IsChecked="True"/>
                    </StackPanel>
                </GroupBox>

                <!-- Import Options -->
                <GroupBox Header="Import Options" Margin="0,0,0,10">
                    <StackPanel>
                        <CheckBox Name="chkSafes" Content="Safes" IsChecked="True"/>
                        <CheckBox Name="chkPlatforms" Content="Platforms" IsChecked="True"/>
                        <CheckBox Name="chkAccounts" Content="Accounts" IsChecked="True"/>
                    </StackPanel>
                </GroupBox>

                <!-- CSV Flow -->
                <GroupBox Header="CSV Flow">
                    <StackPanel>
                        <Button Content="Create sample CSV" Click="CreateSampleCsv_Click" Margin="0,0,0,5"/>
                        <Button Content="Select CSV" Click="SelectExistingCsv_Click" Margin="0,0,0,5"/>
                        <Button Content="Encrypt CSV" Click="EncryptSelectedCsv_Click" Margin="0,0,0,5"/>
                        <Button Content="Run Import" Click="RunImportPCloud_Click"/>
                    </StackPanel>
                </GroupBox>
            </StackPanel>

            <!-- Log Output -->
            <TextBox Name="txtLog" Grid.Column="1"
                     VerticalScrollBarVisibility="Auto"
                     HorizontalScrollBarVisibility="Auto"
                     AcceptsReturn="True"
                     TextWrapping="Wrap"
                     IsReadOnly="True"/>
        </Grid>
    </Grid>
</Window>
"@

Set-Content -Path $xamlPath -Value $newXaml -Encoding UTF8
Write-Host "[OK] MainWindow.xaml updated with CyberArk branding."

Write-Host "`n[DONE] Branding patch applied. Rebuild the solution in Visual Studio."
